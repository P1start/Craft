...
void build_pyramid(int block_type) {
    int old_p1x = p1x;
    int old_p1y = p1y;
    int old_p1z = p1z;
    int old_p2x = p2x;
...
void build_fill(int x, int y, int z, int match, int block_type) {
    if (match == 0) return;
    if (match == block_type) return;
    if (get_block(x, y, z) != match) return;
    set_block(x, y, z, block_type);
    build_fill(x + 1, y, z, match, block_type);
    build_fill(x - 1, y, z, match, block_type);
    build_fill(x, y + 1, z, match, block_type);
    build_fill(x, y - 1, z, match, block_type);
    build_fill(x, y, z + 1, match, block_type);
    build_fill(x, y, z - 1, match, block_type);
}

void build_snow(int x, int y, int z, int radius) {
    for (int px = -radius; px <= radius; px++) {
        for (int py = -radius; py <= radius; py++) {
            for (int pz = -radius; pz <= radius; pz++) {
                if (   px*px + py*py + pz*pz <= radius*(radius + 1)
                    && get_block(px + x, py + y, pz + z) == 1
                    && is_transparent(get_block(px + x, py + y + 1, pz + z))) {
                    set_block(px + x, py + y, pz + z, 9);
                }
            }
        }
    }
    ...
    for (int px = lx; px <= mx; px++) {
        for (int py = ly; py <= my; py++) {
            for (int pz = lz; pz <= mz; pz++) {
                ...
                int ty = py - ly;
                int tz = pz - lz;
                copyBuffer[tx][ty][tz] = get_block(px, py, pz);
            }
        }
    }
}

void build_paste(int hx, int hy, int hz) {
    for (int px = lx; px <= mx; px++) {
        for (int py = ly; py <= my; py++) {
            for (int pz = lz; pz <= mz; pz++) {
                int tx = px - lx;
                ...
                if (btype) set_block(tx + hx, ty + hy, tz + hz, btype);
            }
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (right_click) {
            right_click = 0;
            int hx, hy, hz;
            int hw = hit_test(!replace, s->x, s->y, s->z, s->rx, s->ry,
                &hx, &hy, &hz);
            if (hy > 0 && hy < 256 && is_obstacle(hw)) {
        ...
        if (command_done) {
            ...
            if (success != -1) {
                ...
                //    build_text(buildc + 5, items[item_index]);
                    ...
                    int hx, hy, hz;
                    int hw = hit_test(1, s->x, s->y, s->z, s->rx, s->ry,
                        &hx, &hy, &hz);
                    build_paste(hx, hy, hz);
